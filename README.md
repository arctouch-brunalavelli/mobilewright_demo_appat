...
## **Practical workflow when writing a new test**

1. Decide *what behavior* you want to verify in plain English. ("After I tap Sign in with valid credentials, the login screen should disappear.")
2. Use the inspector (`_inspect.test.ts`) to find selectors for the elements involved.
3. Write the **Act** lines using your Page Object (`await login.signIn()`).
4. Write the **Assert** lines: ask yourself, *"If this behavior is broken, what would I see on screen that’s different?"* — and turn that into one or more `expect(...)` calls.
5. Run it. Make it fail on purpose first (e.g. assert something obviously wrong) to confirm the assertion actually checks something, then fix it.
